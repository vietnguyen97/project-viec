...
class Home extends Component {
    ...
    fetchSearchResult = (updatapage, query) => {
        const numberPage = updatapage ? `&page=${updatapage}` : '';
        const searchURL = `http://27.74.250.96:8384/api/v1/search/suggestion?keyword=${query}${numberPage}`;
        if (this.cancel) {
            this.cancel.cancel();
        }
        ...
        Axios.get(searchURL, {
            ...
            const resultNotFound = !res.data.length ? 'Không tìm thấy trang' : "";
            this.setState({ results: res.data, message: resultNotFound, loading: false })
            console.log(res.id);
        }).catch(err => {
            ...
            return (
                <div>
                    {results.map((result, index) => {
                        return (
                            <li className="row-item-suggestion-popup" key={index}>
                                <a onClick={this.showTest} href="/ket-qua/thong-tin">
                                    {result.title}
                                </a>
                            </li>
                        )
                                        ...
                                        <div className="homepage--input-search">
                                            <div className="input-search__container d-flex justify-content-center">
                                                <div className="w-100 input-search__content collapsed" id="autoComplete__content">
                                                    <input className="form-control" id="autoComplete" ref={input => this.search = input} type="text" name="query" onChange={this.onChange} placeholder="Search ..." tabIndex={1} />
                                                    <ul >
    ...
    //     var result = null;
    //     if(menus.lengh > 0){
    //         result = menus.map((el, index) =>{
    //             return (
    //                 <MenuLink
    //                     key={index}
    //                     label={el.name}
    //                     to={el.to}
    //                     activeOnlyWhenExact={el.exact}
    //                 />
    //             );
    //         })
    //     }
